...
export default function ExperienceItem({ company, role, start, end, duration, logo, url }: ExperienceItemProps) {
  ...
  if (url) {
    return (
      <li>
        <Link 
          href={url}
          className="group grid gap-3 md:grid-cols-[auto,1fr,auto] items-center rounded-2xl px-2 py-3 hover:bg-neutral-50 dark:hover:bg-neutral-900 focus-within:ring-2 focus:outline-none"
        >
          {content}
        </Link>
      </li>
  ...
  return (
    <li className="group grid gap-3 md:grid-cols-[auto,1fr,auto] items-center rounded-2xl px-2 py-3">
      {content}
    </li>
  )
}
